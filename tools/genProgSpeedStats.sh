...
function computeSeed {
    ...
    python3.7 ${CWD}/randomizer.py -r "${ROM}" --randoPreset "${RANDO_PRESET}" --fakeRandoPreset "${FAKE_RANDO_PRESET}" --param "${SKILL_PRESET}" --ext_stats "${SQL}" --runtime ${RUNTIME_LIMIT} > ${LOG}
     if [ $? -eq 0 ]; then
...
for PROG_SPEED in speedrun slowest slow medium fast fastest basic VARIAble; do
    ...
    if [ -n "${FULL}" ]; then
        ...
        FAKE_RANDO_PRESET="Season_Races_Full_${PROG_SPEED}"
    else
        FAKE_RANDO_PRESET="Season_Races_Major_${PROG_SPEED}"
    fi
	...
	while [ ${CUR_JOBS} -lt ${NB_CPU} -a ${CUR_LOOP} -lt ${LOOPS} ]; do
	    computeSeed "${RANDO_PRESET}" "${SKILL_PRESET}" "${FAKE_RANDO_PRESET}" &
	    PIDS="$PIDS $! "
