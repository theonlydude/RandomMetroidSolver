...
class MiniSolver(object):
    ...
    def isBeatable(self, itemLocations):
        locations = []
        ...
        while True:
            if len(locations) == 0:
                return True
            self.areaGraph.getAvailableLocations(locations, self.smbm, self.settings.maxDiff, self.startAP)
            ...
            toCollect = [loc for loc in locations if loc['difficulty'].bool == True and loc['difficulty'].difficulty <= self.settings.maxDiff]
            if len(toCollect) == 0:
                return False
            for loc in toCollect:
                self.smbm.addItem(loc['itemType'])
            locations = [loc for loc in locations if loc['difficulty'].bool == False or loc['difficulty'].difficulty > self.settings.maxDiff]
