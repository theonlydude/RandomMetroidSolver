...
class ComeBack:
    ...
    def handleNoComeBack(self, majorsAvailable, minorsAvailable, cur):
        if self.solver.romConf.majorsSplit == 'Full':
        ...
        if len(self.comeBackSteps) > 0:
            ...
            if self.reuseLastStep(lastStep, solveAreas):
    ...
    def rewind(self, cur):
        ...
        self.solver.rollback(count)
        self.log.debug("Rewind {} steps to {}".format(count, lastStep.cur))
        return True
...
class ComeBackStep:
    ...
    def moreAvailable(self):
        self.log.debug("moreAvailable: cur: {} len(visited): {} len(areas): {}".format(
            self.cur, len(self.visitedSolveAreas), len(self.solveAreas)
        ))
    ...
    def next(self, locations, priorityArea=None):
        ...
        self.log.debug("rewind next, solveAreas: {} visitedSolveAreas: {}".format(self.solveAreas,
                                                                                  self.visitedSolveAreas))
        ...
        outWeight = 10000
        retSolveAreas = {'default': outWeight}
        ...
        # update locs
        self.applyAreaWeight(locations, retSolveAreas)

        return False

    def applyAreaWeight(self, locations, weights):
        # call from comeback step to priorise next solve area after a rollback
        for loc in locations:
            if loc.SolveArea in weights:
                loc.areaWeight = weights[loc.SolveArea]
                ...
                loc.areaWeight = weights['default']
                self.log.debug("rewind loc {} from area {} not in original areas".format(loc.Name, loc.SolveArea))
